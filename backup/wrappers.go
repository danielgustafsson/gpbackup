...
func RetrieveAndBackupTypes(metadataFile *utils.FileWithByteCount, sortables *[]Sortable, metadataMap MetadataMap) {
	...
	BackupShellTypes(metadataFile, shells, bases, rangeTypes, typeMetadata)
	if connectionPool.Version.AtLeast("5") {
...
func BackupShellTypes(metadataFile *utils.FileWithByteCount, shellTypes []ShellType, baseTypes []BaseType, rangeTypes []RangeType, typeMetadata MetadataMap) {
	gplog.Verbose("Writing CREATE TYPE statements for shell types to metadata file")
	PrintCreateShellTypeStatements(metadataFile, globalTOC, shellTypes, baseTypes, rangeTypes, typeMetadata)
}
