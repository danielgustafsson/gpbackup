...
ALTER TYPE public.base_type
	...
	Describe("PrintCreateShellTypeStatements", func() {
		...
		It("prints shell type for a shell type", func() {
			backup.PrintCreateShellTypeStatements(backupfile, toc, []backup.ShellType{shellOne}, []backup.BaseType{}, []backup.RangeType{}, emptyMetadataMap)
		...
		It("prints shell type with an owner and comment", func() {
			backup.PrintCreateShellTypeStatements(backupfile, toc, []backup.ShellType{shellOne}, []backup.BaseType{}, []backup.RangeType{}, typeMetadataMap)
			testutils.ExpectEntry(toc.PredataEntries, 0, "public", "", "shell_type1", "TYPE")
			expectedEntries := []string{"CREATE TYPE public.shell_type1;",
				"COMMENT ON TYPE public.shell_type1 IS 'This is a type comment.';",
				"ALTER TYPE public.shell_type1 OWNER TO testrole;",
				"SECURITY LABEL FOR dummy ON TYPE public.shell_type1 IS 'unclassified';"}
			testutils.AssertBufferContents(toc.PredataEntries, buffer, expectedEntries...)
		})
		It("prints shell type for a base type", func() {
			backup.PrintCreateShellTypeStatements(backupfile, toc, []backup.ShellType{}, []backup.BaseType{baseOne, baseTwo}, []backup.RangeType{}, emptyMetadataMap)
		...
		It("prints shell type for a range type", func() {
			backup.PrintCreateShellTypeStatements(backupfile, toc, []backup.ShellType{}, []backup.BaseType{}, []backup.RangeType{rangeOne}, emptyMetadataMap)
